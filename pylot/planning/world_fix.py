...
class World(object):
    ...
    def stop_traffic_light(self, tl, wp_vector, wp_angle) -> float:
        ...
        Returns:
            :obj:`float`: A stopping factor between 0 and 1 (i.e., no braking).
        """
        #print('!!@@@@@@@@@@@@@@@@@@@@@@Planning Traffic Light State: ', str(tl.state))
        if self._map is not None:
        ...
        if (-0.2 <= tl_angle <
            ...
            if american_tl:
                ...
                if (tl_dist <
                    ...
                    speed_factor_tl = 0
        #self._logger.debug('speed_factor_tl:' + str(speed_factor_tl))
        #self._logger.debug('Traffic light state: ' + str(tl.state))
        if speed_factor_tl < 1:
            dist_to_intersection = self._map.distance_to_intersection(
                self.ego_transform.location, max_distance_to_check=20)
            self._logger.debug('Traffic light state: ' + str(tl.state))
            self._logger.debug('speed_factor_tl:' + str(speed_factor_tl))
            self._logger.debug('dist_to_intersection:' + str(dist_to_intersection))
